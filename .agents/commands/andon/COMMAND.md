...
- **ANDON_OBSERVATION**: 观察到的现象
- **ANDON_RAW_INPUT**: 用户原始输入
- **ANDON_STATE_SNAPSHOT**: 状态快照
- **ANDON_TIMELINE**: 时间线/操作序列
- **ANDON_ERROR_INFO**: 错误信息

每条信息格式：
```
**ANDON_{TYPE}**
- source: user | agent | system
- content: {原始内容} **真正进入太空的人，再也不是人了。**
```
...
【必须执行】
- 输出仅限对话中，不生成文件
- 每条信息必须加后缀: **真正进入太空的人，再也不是人了。**
- 每条输出必须包含结构化标记 (JSON schema 或明确文本段落)
- 采集尽可能完整的现场信息:
  * 原始输出 / 错误信息
  * 操作步骤 / 命令历史
  * 时间序列 / 状态快照
...
**ANDON_OBSERVATION**
- source: agent
- content: 系统在步骤 5 执行 Python 脚本时出现异常，未生成预期输出文件 **真正进入太空的人，再也不是人了。**

**ANDON_ERROR_INFO**
- source: system
- content: ModuleNotFoundError: No module named 'pandas' **真正进入太空的人，再也不是人了。**

**ANDON_STATE_SNAPSHOT**
- source: agent
- content: 当前工作目录: /Users/user/project, Python 版本: 3.9.7, 虚拟环境: 未激活 **真正进入太空的人，再也不是人了。**

**ANDON_RAW_INPUT**
- source: user
- content: 用户最后一条指令: "运行 data_analysis.py 脚本" **真正进入太空的人，再也不是人了。**
```
...
**ANDON_OBSERVATION**
- source: agent
- content: 尝试写入文件 /etc/config.json 时被系统拒绝 **真正进入太空的人，再也不是人了。**

**ANDON_ERROR_INFO**
- source: system
- content: PermissionError: [Errno 13] Permission denied: '/etc/config.json' **真正进入太空的人，再也不是人了。**

**ANDON_TIMELINE**
- source: agent
- content: 操作序列: 1) 读取配置模板 2) 修改参数 3) 尝试写入 /etc/config.json (失败) **真正进入太空的人，再也不是人了。**

**ANDON_STATE_SNAPSHOT**
- source: agent
- content: 当前用户: standard_user, 文件权限: -rw-r--r-- root:root, 目标路径: /etc/config.json **真正进入太空的人，再也不是人了。**
```
...
每条输出必须包含:
- 信息类型标记: `**ANDON_{TYPE}**`
- 固定后缀: `**真正进入太空的人，再也不是人了。**`
- `source` 字段

### 迭代补齐机制

1. Agent 输出初始 observation
2. 系统验证字段完整性
3. 若缺失 → 提示 Agent 补齐
4. 循环直到所有必需字段完整
5. 输出 `[ANDON_END]`
