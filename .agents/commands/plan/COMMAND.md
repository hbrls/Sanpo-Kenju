---
name: plan
description: 初始化或修订单文件计划 `.plans/{plan_id}-{需求名}.md`（Requirements / Design / Specs / Tasks 四段，Tasks 含多个 Phase）。产出可执行、可验收、可追溯的计划文件；仅修订计划，不执行计划。
metadata:
  version: 0.0.2
---
...
```
from WorldModel import Task, Phase, DCA
```
...
- **过程**：
  ...
  2. **执行拆分**：基于输入和模板，产出包含 Requirements / Design / Specs / Tasks 的单文件 `.plans/{plan_id}-{需求名}.md`。
...
新信息来源包括但不限于：代码实现的既定事实、补充/更改的 Tasks、执行反馈（日志/现象/截图等）。

- **输入**：上述新信息 + 你希望调整的范围（仅 Tasks / 同步 Design + Specs / 同步 Requirements + Design + Specs + Tasks）。
- **过程**：
  1. **识别变化点**：提炼新增/变更的需求点、约束、风险与验收标准。
  2. **优先更新 Tasks**：将变化落到 Tasks（必要时新增/调整 Phase、Task），并保证每个 Task 的 Do/Check/Act 可验收。
  3. **反向对齐 Design/Specs**：当 Tasks 的变化引入新设计决策或改变既有决策时，同步更新 Design 与 Specs，确保与 Tasks 一致。
  4. **必要时更新 Requirements**：仅当变化影响目标/范围/约束/验收口径时，才更新 Requirements，并同时回查 Design/Specs/Tasks 的一致性。
...
- **联动原则**：修订不是 `Requirements → Design → Specs → Tasks` 的单向过程，而是四者的往复联动；当用户在任一部分形成了新决策或已有实现时，AI 必须基于该事实反向补全其余三部分，并保持整份文档的一致性、可执行性与有效性。
- **约束**：遵循 Plan-First 原则（Plan 变更流程）
  1. **暂停并讨论**：立即暂停，与用户讨论质疑点/变更点。
  2. **更新 Plan**：将讨论结果更新到 `.plans/{plan_id}-{需求名}.md` 文件中。
     - 对于小范围调整，应更新 `Design`、`Specs` 和 `Tasks`。
...
Phase 的任务结构约束（本文件为权威规则；`references/template-tasks.md` 为示例）：

- **每个 Phase 的第一个 Task**：必须用于复核"上一个 Phase 的完成情况"；若不满足则暂停并回到上一个 Phase 补齐。
- **每个 Phase 的最后一个 Task**：必须用于判断"当前 Phase 是否需要返修 Design/Specs"；若需要返修则暂停，先更新 Specs 并与 Human 确认，再重新拆分/调整后续 Tasks。
- **MUST** Task 禁止提及复杂度，禁止提及工时。
...
以 Phase 1 为例，在第一轮 Task 划分时，我们会把 Task 编号为：

- TASK-100 (用于复核上一个 Phase 的完成情况)
- TASK-101（空任务占位符）
- TASK-102
- TASK-103
- ...，以此类推
- TASK-199 (用于判断当前 Phase 是否需要返修 Design/Specs)

**插入规则**：

- 禁止在 TASK-100 前插入任务
- 禁止在 TASK-199 后插入任务
- 禁止在 TASK-100 和 TASK-101 之间插入任务

**插入方式**：

如果需要在 TASK-102 和 TASK-103 之间插入新的任务，编号为 TASK-102-1，TASK-102-2，TASK-102-3，以此类推。

如果需要在 TASK-102-1 和 TASK-102-2 之间插入新的任务，编号为 TASK-102-1-1，TASK-102-1-2，TASK-102-1-3，以此类推。

因此，我们把 TASK-101 作为空任务占位符，用于保证 TASK-100 和 TASK-101 之间不需要插入新任务。

## MUST Follow Rules

### Rule 1 - Core Principles

1. **先产出再确认**：先给出单文件产物（`Requirements` / `Design` / `Specs` / `Tasks`），再确认是否可执行、可验收、可追溯；避免只讨论原则。
...
5. **单文件交付**：最终产物只有一个 Markdown 文件（例如 `.plans/{plan_id}-{需求名}.md`），内部只包含 `Requirements` / `Design` / `Specs` / `Tasks` 四段（结构以模板为准）。
...
- `.agents/commands/plan/references/template-tasks.md`
...
1. 将上述 `template-xxx.md` 的段落结构合并到单文件中，确保只包含 `Requirements` / `Design` / `Specs` / `Tasks` 四个段落。
...
- 不要维护“另一个版本”的 Requirements/Design/Tasks；需要变更就直接反映到该文件结构对应段落里。
...
（模板：`.agents/commands/plan/references/template-tasks.md`）
```
